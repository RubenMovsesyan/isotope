...
@group(2) @binding(1)
var<storage> global_orientation: vec4<f32>;
...
fn main(
    ...
    // let combined_rotation = quat_norm(hamilton_prod(global_orientation, instance.rotation));

    // Rotate the point first
    let conj = quat_conj(global_orientation);

    let rot: vec4<f32> = hamilton_prod(
        hamilton_prod(
            global_orientation,
            vec4<f32>(model.position + (model.normal * 0.01), 0.0),
        ),
        conj
    );
